(function () {
    ...
    var selectEmpleado = document.getElementById('filtro-empleado');

    if (!contenedor || !form) return;

    var ordenActual = 'fecha_desc';
    var timeout = null;

    function construirURL(page) {
        var params = new URLSearchParams();
        if (inputQ.value.trim()) params.set('q', inputQ.value.trim());
        if (selectEmpleado.value) params.set('empleado', selectEmpleado.value);
    ...
    function cargar(page) {
        fetch(construirURL(page), {
            ...
            .catch(function (err) { console.error('Error al cargar informes:', err); });
    }
